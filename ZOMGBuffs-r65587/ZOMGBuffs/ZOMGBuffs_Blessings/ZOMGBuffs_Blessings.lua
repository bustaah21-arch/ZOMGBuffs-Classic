...
local BS = LibStub("Babble-Spell-2.2")		-- LibBabble-Spell-3.0
...
z:CheckVersion("$Revision: 65463 $")

local SM = z.SharedMedia
...
-- GetUnitPalaBuffs
local function GetUnitPalaBuffs(unitid)
...
function zb:CheckBuffs()
	...
	local classesCheckAbsent = new()
	local totalPresent = 0
	local classNeedCount = 0
	local notNeeding = new()
	...
	for unit in roster:IterateRoster() do
		...
		elseif (absent) then
			...
			classesCheckAbsent[unitclass] = (classesCheckAbsent[unitclass] or 0) + 1
			notNeeding[unitclass] = (notNeeding[unitclass] or 0) + 1
			classesCheckPresent[unitclass] = (classesCheckPresent[unitclass] or 0) + 1
			totalPresent = totalPresent + 1
		end
	end
	...
	for unitid,unitclass in pairs(unitList) do
		...
		if (needType) then
			local myBuff, otherBuffs, myBuffTimeLeft = GetUnitPalaBuffs(unitid)
			local ignore
				...
				if (UnitExists(masterUnit) and UnitIsVisible(unit.unitid) and UnitCanAssist("player", unit.unitid)) then
					...
					if (masterClass) then
						if ((classesCheckPresent[masterClass] or 0) == (z.classcount[masterClass] or 0)) then
							...
							if (needType) then
								local myBuff, otherBuffs, myBuffTimeLeft = GetUnitPalaBuffs(unit.unitid)
...
--self:Print("limitToClass = %s, classNeedCount = %s, classcount = %s, classesCheckAbsent = %s, classesCheckPresent = %s, notNeeding = %s", tostring(limitToClass), tostring(classNeedCount), tostring(limitToClass and z.classcount[limitToClass] or "n/a"), tostring(classesCheckAbsent[limitToClass] or "n/a"), tostring(classesCheckPresent[limitToClass] or "n/a"), tostring(notNeeding[limitToClass] or 0))
-- and (classesCheckPresent[limitToClass] == 1 or (classNeedCount > 1 and (notNeeding[limitToClass] or 0) >= zb.db.char.min))
-- and not (classNeedCount == 1 and (notNeeding[limitToClass] == ((z.classcount[limitToClass] - (classesCheckAbsent[unitclass] or 0)) - 1)))
	...
	if (limitToClass and classNeedCount >= zb.db.char.min) then
		for unitid,needed in pairs(ltcUnits) do
			...
			if (not failedRecently and IsSpellInRange(ltcSpell, unitid) == 1) then
				z:Notice(format(L["Class %s needs %s"], z:ColourClass(limitToClass), z:ColourBlessing(ltcType, true)), "buffreminder")
	...
	elseif (singleNeedSpell) then
		z:Notice(format(L["%s needs %s"], z:ColourUnit(singleNeedUnit), z:ColourBlessing(singleNeedType)), "buffreminder")
...
function zb:SetTemplateTypeClass(unit, type)
	...
	if (mod and z.db.profile.info) then
		self:Print(L["Modified template: %s: %s"], z:ColourClass(unit.class), z:ColourBlessing(type))
	end
end
	...
	if (difference and z.classcount[class] > 1) then
		if (template[name] ~= type) then
			...
			if (z.db.profile.info) then
				self:Print(L["Modified template: %s: %s"], z:ColourUnit(unitid), z:ColourBlessing(type))
			end
	...
	if (s) then
		...
		if (b) then
			...
			if (b.class) then
				...
				if (unitid) then
					self:Print(L["%s on %s (%s%d|r)"], z:ColourBlessing(b.type, true), z:ColourClassUnit(unitid), colourCount, count)
						...
						if (unit.class == unitclass) then
							...
							if (unitNeed) then
								if (unitNeed ~= b.type) then
									self:Print(L[" %s now needs %s"], z:ColourUnit(unit.unitid), z:ColourBlessing(unitNeed))
								else
			...
			self:Print(L["%s on %s"], z:ColourBlessing(b.type), tostring(z:ColourUnit(unitid)))
		end
	end
end
		...
		if (show and self.db and self.db.char.icons) then
			...
			if (#list > 0) then
				...
				for i,name in ipairs(list) do
					...
					icon.icon:SetTexture(BS:GetSpellIcon(singleSpell))
