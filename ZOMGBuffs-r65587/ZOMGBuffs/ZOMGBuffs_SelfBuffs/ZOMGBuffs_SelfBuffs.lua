...
local gratuity = LibStub("Gratuity-2.0", true)
...
local gratuity, durMatch, encMatchHour, encMatchMin, encMatchSec
function zs:GetCurrentItemEnchant(slot)
	...
	if (hasEnchant) then
		if (gratuity) then
			...
			local ok, cd = gratuity:SetInventoryItem("player", slot)
			if (ok and cd) then
				...
				if (encLine) then
					local left = gratuity:GetLine(encLine)
					if (left) then
		...
		if (not self.activeEnchant or self.activeEnchant < GetTime() - 4) then
			...
			if (not hasEnchant or Expiration / 1000 < self.db.char.rebuff.default) then
				if (InCombatLockdown() or self:SetupForItem(slot, spellOrItem)) then
					...
					z:Notice(format(L["You need %s on |c00FFFF80%s|r"], z:LinkSpell(spellOrItem), itemName), "buffreminder")
					return true
				end
			end
		end
				...
				if (not myBuffs or not myBuffs[k]) then
					if (v.auto()) then
						if (not UnitOnTaxi("player")) then
							...
							z:Notice(format(L["You need %s"], z:LinkSpell(k)), "buffreminder")
		...
		if (v.who == "weapon") then
			...
			if (spell or item) then
				...
				if (item) then
					...
					spellIcon = BS:GetSpellIcon(spell)
				end
...
function zs:TooltipUpdate(cat)
	if (template and self.classBuffs) then
		...
		for i,key in ipairs(list) do
			...
			if (template[key]) then
				local checkIcon = BS:GetSpellIcon(key)
