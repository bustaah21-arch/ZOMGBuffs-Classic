...
local function ColourSpellFromKey(key)
	...
	if (wow24) then
		local icon = BS:GetSpellIcon(key.list[1])
		if (icon) then
			return format("|T%s:14|t%s%s|r", colour, name), colour
		end
	end
...
function zg:CheckBuffs()
	...
	if (z.db.profile.waitforraid > 0 and not skipClassBuffs) then
...
function zg:OnModuleInitialize()
	...
	elseif (playerClass == "WARLOCK") then
		self.buffs = {
			SEEINVIS = {
				o = 1,
				list = {BS["Detect Greater Invisibility"]},
				alternatives = {BS["Detect Invisibility"], BS["Detect Lesser Invisibility"]},
				colour = {0.7, 0.2, 0.2},
				limited = true,						-- Allow limited targets config
				keycode = "seeinvis",
			},
		...
		if (self.tickColumns) then
			for i,key in ipairs(self.tickColumns) do
				...
				if (relative) then
					...
					icon:SetTexture(BS:GetSpellIcon(handler.spell))
	...
	if (self.lookup) then
		...
		for button = 1,5 do
			...
			if (spell == name) then
				...
				if (find and find.list[2]) then
					if (find.list[1] == name or find.list[2] == name) then
						return true
					end
				end
			end
...
function zg:SayWhatWeDid(icon, spell, name, rank)
	...
	if (s) then
		...
		if (found) then
			...
			if (unit and found.list and spell == found.list[2]) then
				local subgroup = z:GetGroupNumber(unit.unitid)
...
do
	...
	local function iconUpdateTooltip(self)
		...
		GameTooltip:AddLine(format(L["%s on %s"], ColourSpellFromKey(buff), z:ColourUnitByName(self.target)))
	...
	local function iconOnShow(self)
		...
		self.posindex = zg:GetNewIconIndex()
		self:SetPosition()
		self:UpdateCooldown()
	end
	...
	local function iconOnHide(self)
		...
		self.spell = nil
		self.target = nil
		zg:FreeIconIndex(self.posindex)
		self.posindex = nil
	end
	...
	local function iconSetTrackerAttributes(self, target, key)
		...
		self.count:Hide()
		self.name:Hide()
		self.icon:SetTexture(BS:GetSpellIcon(spell))
	...
	if (template) then
		...
		for i,k in ipairs(list) do
			...
			if (GetSpellCooldown(key.list[1])) then
				...
				local checkIcon = BS:GetSpellIcon(key.list[1])
