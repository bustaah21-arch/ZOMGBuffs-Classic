...
function man:SetTalentNamesAndIcons(class)
	if (not talentIconCache[class]) then
		local name1, name2, name3, icon1, icon2, icon3
		name1, icon1 = GetTalentTabInfo(1, true)
		name2, icon2 = GetTalentTabInfo(2, true)
		name3, icon3 = GetTalentTabInfo(3, true)
	...
	if (class) then
		for i = 1,6 do
			...
			if (Type) then
				...
				local icon = BS:GetSpellIcon(classSpell or singleSpell)
...
function man:GetException(name, row, col)
	if (not man.configuring) then
		local pala = self:GetPalaFromRow(row)
		if (pala) then
			return z:GetBlessingFromType(pala.template[name])
		end
	end
end
...
function man:UnitContextMenu(row, col)
	...
	if (list or self:HasException(row, col)) then
		...
		local function getFunc(k)
			return self:GetException(k[1], contextRow, contextCol) == k[2]
		end
		local function setFunc(k, onoff)
			self:SetException(k[1], contextRow, contextCol, k[2])
		end
